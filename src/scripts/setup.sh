...
chmod +x ../scrape_jobs.py ./schedule_scrape_jobs.sh
...
echo "Termux requires a full restart. Completely exit the app now."
echo "Upon restart, execute ./<Job-Listing-Scraper-x.x>/src/scripts/schedule_scrape_jobs.sh" to begin running the job scraper.
sleep 30
